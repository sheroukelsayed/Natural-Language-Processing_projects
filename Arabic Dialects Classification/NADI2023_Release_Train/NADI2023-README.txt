...
For MADAR-18.tsv: it contains  the original MADAR train columns (sentID.BTEC, split, lang, sent), in addation we add the belong city-level (column city) as well as the country-level labels (column country).
For NADI-2020-TWT.tsv and NADI-2021-TWT.tsv: it contains  the original NADI 2020 and 2021 columns (#1_tweet_ID, #2_tweet_content, #3_country_label, #4_province_label). 

For subtask 2 and subtask 3 we only provide Dev data data are in tsv files:
(1) Subtask 2 (Closed track): Dialect to MSA translation:
    ./Subtask_2/Subtask2/NADI2023_Subtask2_DEV.tsv
    - You can download MADAR data at (https://camel.abudhabi.nyu.edu/madar-parallel-corpus/)
    - You can only use splits of Corpus-6-train and Corpus-6-test-corpus-26-train to train your model. 

  You can use this dictionary to map cities to countries in MADAR-parallel-corpus.  
  city2country={'Salt':'Jordan', Aleppo':'Syria', 'Baghdad':'Iraq', 'Jerusalem':'Palestine', 'Fes':'Morocco', 'Doha':'Qatar', 'Beirut':'Lebanon', 'Aswan':'Egypt', 'Damascus':'Syria', 'Muscat':'Oman', 'Alexandria':'Egypt', 'Rabat':'Morocco', 'Basra':'Iraq', 'Tripoli':'Libya', 'Amman':'Jordan', 'Tunis':'Tunisia', 'Benghazi':'Libya', 'Sanaa':'Yemen', 'Jeddah':'Saudi_Arabia', 'Mosul':'Iraq', 'Sfax':'Tunisia', 'Algiers':'Algeria', 'Cairo':'Egypt', 'Khartoum':'Sudan', 'Riyadh':'Saudi_Arabia'}

(2) Subtask 3 (Open track): Dialect to MSA translation:
    ./Subtask_2/Subtask2/NADI2023_Subtask3_DEV.tsv

For DEV files of Subtask 2 and 3, we provide the following information:
- The first column (#1_id) contains an ID representing the data point/tweet.
...
- The fourth column (#4_target_msa) contains a translated target text in MSA.
...
(2) NADI2023_Subtask2_DEV.tsv and NADI2023_Subtask3_DEV.tsv contain 400 data points for each one.
...
Training Example
-------------------------------------
We provide a **Colab finetuning example** to demonstrate how to finetune pre-train LMs (like MARBERT and Ara-T5) on the tasks: https://colab.research.google.com/github/UBC-NLP/nadi/blob/main/NADI2023.ipynb
